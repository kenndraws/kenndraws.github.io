...
const stateAbbreviations = ['AL', 'AK', 'AS', 'AZ', 'AR', 'CA', 'CO', 'CT', 'DE', 'DC', 'FM', 'FL', 'GA',
...
function setElementValidity(fieldName, valid, message) {
    let $field = $(fieldName);
    let el = $field.get(0);
    if (valid) el.setCustomValidity('');  //sets to no error message and field is valid
    else el.setCustomValidity(message);   //sets error message and field gets 'invalid' stat
